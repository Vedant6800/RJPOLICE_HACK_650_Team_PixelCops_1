...
//getting users for the admin panel
function getUsers() {
    global $db; // Assuming $db is your database connection variable

    // Sample query to get user names and email addresses
...
function validateUpdateForm($form_data, $image_data)
{
    ...
    if ($image_data['name']) {
        ...
        if ($type != 'jpg' && $type != 'jpeg' && $type != 'png' && $type != 'webp') {
            $response['msg'] = "only jpg,jpeg,png,webp images are allowed";
            $response['status'] = false;
